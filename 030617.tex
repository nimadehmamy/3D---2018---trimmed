...
It does so by assigning spring-like attractive forces between linked node pairs and weak repulsive forces between all node pairs %. Thus, nodes and links move to minimize the energy
...
We, therefore, model each link as a rubber with fixed diameter and that are unable to cross each other.%band which have a volume and do not cross each other
...
% \end{figure}
%
In Fig. \ref{fig:phase-compare} we illustrate the ELM layout by applying our algorithm to a small %Barabasi-Albert (BA) 
...
When links are thick ($r_L = O(r_N)$) %for $N_l$ links of thickness $r_l$ we have 
the layout will be comprised of closely packed links and the cross sectional area of the layout, $\pi L^2$, is comparable to the cross section of all links ($ N_l r_L^2 \sim L^2$). % (for $N_l$ links of thickness $r_l$).%, so $R \sim N_l^{1/2} r$. 
