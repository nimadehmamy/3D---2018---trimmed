...
%\begin{table}%[]
    ...
    \hline
        ...
        \hline 
Abstract (words) & 200-300 &  500 & 327 \\
\hline 
Summary & 1 sentence ``we show'' & 150 words & - \\ 
\hline 
Text (words) & 1500 & 3000 & 2164\\
\hline 
...
\hline 
    ...
    {\bf \scriptsize Our current text is $\sim$2200 words and satisfies the Article format, but would need significant cuts to fit Letter. With the font and page size of PRL, the text alone becomes 3 pages. Adding figures would make it roughly 5. }
    %\label{tab:nature}
...
% \begin{document}
{\bf 
In many physical networks, from neurons in the brain \cite{kasthuri2015saturated,oh2014mesoscale} to 3D integrated circuits \cite{wong2007monolithic,shulaker2014monolithic} %to bundles of cables carrying power in infrastructural systems 
...
extensive theoretical frameworks used to characterize the structure of real networks\cite{barrat2008dynamical,newman2010networks,caldarelli2012networks}, %none of the currently used network models take into account this physicality. 
...
\begin{figure}
    ...
    \caption{
    \scriptsize
    {\bf(A) Modeling Framework:} We model each link as a stretched, flexible rubber band, %which can be thought of as
    corresponding to many short springs connected to each other, pulled apart by
    elastic forces $F_{el}$. % of each spring is shown by black arrows.
    Links exert a repulsive force $F_{LL}$  on each other that falls sharply at radii larger than $r_L$. 
    While in FDL the links cross each other, in ELI and FUEL are prohibited. 
    %First and second plot show how the ELI resolves crossing (red) links. 
    ...
    %The link thickness corresponds to the actual thicknesses, while the node size is chosen based on proper visibility. Choosing short-range repulsion for nodes is crucial for getting a perfect lattice layout. Long range forces, e.g. electrostatic,  distort the layout at the boundaries. 
    showing the evolution of the total link length over time during the simulation. % with several snapshots of the actual layout shown above each stage of the evolution. 
    The simulation started from a completely random layout and we use simulated annealing to find the final layout.
    The thermal noise %results in a very large total edge length, but it 
    ...
    {\bf (F)} The average link length versus $r_L/r_N$ for three networks with different $N$ and $L$. % near the phase transition. 
    The geometrical phase transitions are observed at different values of $r_L/r_N$. 
    {\bf (G)} Rescaling the ratio of $r_L/r_N$ using the prediction of \eqref{eq:trans}  collapses all curves, % on similar curves with phase transition happening around near $1$, 
...
Fig. \ref{fig:crs-lat}E shows how FUEL finds the correct 3D configuration of a lattice, %(SI sec.\ref{ap:np}), 
...
As FDL ignores the physical dimensions of the nodes and links (Fig. \ref{fig:crs-lat}),  it suffers from multiple link and node crossings. % for nonzero $r_L$ (Fig. \ref{fig:phase-compare}A), which it cannot resolve. %\ref{fig:crs-lat}A). 
The number of such conflicts increases linearly with $r_L$, and only saturates for large $r_L$ (Fig. \ref{fig:phase-compare}A), a behavior analytically predicted by a geometric model (SI, sec. \ref{ap:cross}). %\cite{kabourov2015spring}. 
...
We find that the obtained layouts undergo a visually detectable geometrical transition as we increase the link thicknesses
\outNim{ % 071917
\begin{figure}
    \centering
...
\caption{%3D layouts  using ELM and E-ELM, and their phase diagram.
    \scriptsize {\bf Layout Transition in Networks:} %{\bf (A)} 
...
The observed linear increase in $\be{l}$ and the $1/r_L$ decrease in $\be{C}$ is consistent with isometric scaling, indicating that the layouts for different $r_L$ in the strongly interacting phase are structurally similar to each other after being rescaled by the scale parameter $r_L$ %and are scaling linearly with the $r_L$. 
...
\begin{figure}
    ...
    \caption{\scriptsize {\bf (A)} When the links are thin, i.e. $r_L\ll r_N$, they interact only weakly with each other --the weakly interacting regime. 
    In this regime, the range of the node repulsion $r_N$ forces the nodes to stay within $2r_N$ of their neighbors, and links only act as bonds keeping the network together.
...
%Given its fundamental geometrical nature, we expect to apply to all models that consider the physical nature of nodes and links 
(Fig. \ref{fig:trans}C).  
...
\begin{equation}
    %\mbox{}\int d^3x  
    T_{\mu\nu} \approx %\left[ k -{A_L\pi^2\be{l}}   \right]
    f(k,A_L,\be{l})\delta_{\mu\nu} \label{eq:Tstrong}
,\end{equation}
%
describing a hydrodynamic stress with pressure $p = f(k,A_L,\be{l})$ %$ p =  k -{A_L\pi^2\be{l}}$. 
} %%%%%
...
\begin{figure}
    ...
    The weakly interacting phase shows a slowly increasing level of stress for increasing thicknesses. %, but the value also fluctuates a lot at different orientations (SI sec.\ref{ap:stress}). 
...
\begin{figure}
    ...
    {\bf (B)} Examples of layouts showing total stress (more red means more stress).  
    Each network corresponds to the  link thicknesses in the plots below them.
    Notice that in the weakly interacting (left) most of the stress is concentrated in a few nodes and most links show very little stress. 
    %Near the phase transition (middle) both nodes and links show a stress, and 
    In the strongly interacting phase (right) almost all of the stress is in the links. 
    {\bf (C)} Total Stress of different topologies as a function of link thickness. Because networks are inhomogeneous objects, stress is averaged over 50 random network orientations. The weakly interacting phase shows a very slowly increasing level of stress for increasing thicknesses. %, but the value also fluctuates a lot at different orientations (SI sec.\ref{ap:stress}). 
...
\begin{figure}
    ...
    Left: Examples of tensile ($\parallel$, red) and transverse ($\perp$, blue) stress in networks in the strongly interacting (top) and weakly interacting (bottom) phases. 
    ...
    In the strongly interacting phase, all links show stress, whereas in the weakly interacting phase only a few have significant stress. 
    The strongly interacting phase shows more parallel tensile stress at the walls and more transverse stress in the mid parts. 
    The weakly interacting phase, on the other had, shows mostly tensile stress in links, mostly due to change in their length via the compression.}
    \label{fig:stress}
\end{figure}
}%%%%%
...
\begin{figure}
    ...
    {\bf (C)} We have 3D print of the flavor network using a commercial 3D printer, allowing the observer to inspect the full 3D structure of the network. %  \cite{ahn2011flavor}. 
...
%\newpage
%\section*{Discussion}
In summary, by exploring the layout geometry of physical networks we observe two distinct phases: a weakly interacting phase, where the overlap between the nodes and links can be avoided via local link rearrangements; and a strongly interacting phase, whose layout is shaped by %link crossings.
...
Many aspects of such networks are studied  as part of ``gelation'' processes in polymer physics (see \cite{de1979scaling} chapter IV for instance). 
...
We start by showing the limitations of FDL when links have thicknesses (sec. \ref{ap:cross}.) 
...
We then discuss in sec. \ref{ap:np}  how this system has a very large number of local minima and that we need to resort to methods such as simulated  annealing to escape unfavorable local minima and find reasonable layouts (sec. \ref{ap:annealing}.)  
...
This requires significant change to the Gaussian potential and becomes a version of the Gay-Bern potential \citep{gay1981modification}  (sec. \ref{ap:ell}.)
We then discuss the order parameters that we use to quantify the phases of networks at different values of link thicknesses and node sizes (sec. \ref{ap:phase}.) 
After that we derive the stress tensor for the layouts (sec. \ref{ap:stress}.) We show that in the weakly interacting phase, layouts have stress properties similar to solids, whereas layouts in the strongly interacting phase behave more like a gel. 
...
The repulsive terms from $V_{NN}$ in \eqref{eq:T} resemble the pressure tensor discussed in \cite{stillinger1984packing} and used for describing jamming phenomena and stress networks in granular material  \cite{o2002random,o2003jamming}.  
...
We find that, as predicted by the theory, $\sigma_\parallel/\sigma_\perp$ fluctuates significantly for node stress at almost all (except at very high) link thicknesses, indicating that it does not satisfy the hydrostatic condition (Fig. \ref{fig:stress2}).%(SI sec.\ref{ap:stress}).
