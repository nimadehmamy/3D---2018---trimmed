...
\documentclass[endfloats,nofootinbib,preprint,floatfix,titlepage,superscriptaddress,linenumbers]{revtex4-1} % linenumbers ,endfloats,floatfix
...
% \begin{document}
\outNim{
In many physical networks, from neurons in the brain to 3D integrated circuits or underground hyphal networks, the nodes and links are physical objects unable to cross each other.
These non-crossing conditions constrain their layout geometry and affect how these networks form, evolve and function, limitations ignored by the  extensive theoretical framework used to characterize real networks. 
Indeed, the currently used network layout methods are variants of the Force-Directed Layout (FDL) algorithm, 
...
For small link thicknesses, we observe a weakly interacting phase where the layout avoids the numerous potential link crossings via local link rearrangements, without altering the overall layout geometry. 
Once the link thickness exceeds a critical threshold, a strongly interacting phase emerges, where multiple geometric quantities, from the total link length to the link curvature, scale with link thickness. 
...
which assumes dimensionless nodes and links, hence are inherently unable to reveal the geometry of densely packed physical networks.
Here, we develop a modeling framework 
%inspired by manifold dynamics 
that accounts for the physical reality of nodes and links, allowing us to 
explore how the non-crossing conditions affect the geometry of the network layout. 
...
The network defined by \eqref{eq:Vfull}--\eqref{eq:Langevin0} has an uneven potential energy landscape \cite{bouchaud1998out} %\cite{parisi2002physical} 
...
% \includegraphics[height=.77\textheight,trim=0 1cm 0 6cm]{fig-09-19/3d-phase-compare-111917-2.pdf}
\includegraphics[height=.7\textheight,trim=0 1cm 0 6cm]{fig-09-19/3d-phase-compare-041218-3.pdf}
\caption{%3D layouts  using ELM and E-ELM, and their phase diagram.
    ...
    { ({\bf C}) The average link curvature increases slowly %remains fairly constant
    ...
    The geometrical phase transitions are at different $r_L/r_N$ values. 
    {\bf (L)} Rescaling the ratio of $r_L/r_N$ using \eqref{eq:trans}  collapses the transition point, % on similar curves with phase transition happening around near $1$, 
    confirming the validity of \eqref{eq:trans}.
    }%color
    \vspace{-2cm}
          }   
...
% \includegraphics[width = \textwidth,trim=1cm 0cm 1cm 0cm]{fig-09-19/3d-phase-compare-041218-4.pdf}
% \includegraphics[width = \textwidth,trim=1cm 0cm 0cm 0cm]{fig-09-19/3d-phase-compare-041218-5.pdf}
% \includegraphics[width = \textwidth,trim=1cm 0cm 0cm 0cm]{fig-09-19/3d-phase-compare-042418-5.pdf}
% \includegraphics[width = \textwidth,trim=1cm 0cm 0cm 0cm]{fig-09-19/3d-phase-compare-042418-6.pdf}
% \includegraphics[width = \textwidth,trim=1cm 0cm 0cm 0cm]{fig-09-19/3d-phase-compare-042418-7.pdf}
...
\caption{%3D layouts  using ELM and E-ELM, and their phase diagram.
    ...
    { ({\bf C}) The average link curvature rises slowly %remains fairly constant
    in the weakly interacting regime, until the transition region where it gradually reaches a peak around the transition point. 
    In the strongly interacting regime the curvature falls linearly with increasing $r_L/r_N$. 
    }%%%%%%%
    % Once scaled by the link length, $\be{C}$ is constant in the strongly interacting regime. 
    ({\bf D}) The relaxation time of the simulations a grows significantly at the transition point. % similar to that observed in second order-like phase transitions. 
    ...
    %Almost all links will feel the push of other links along most of their length. 
    % When the total volume of these links becomes comparable to the volume of the bounding sphere of nodes, the layout transitions to the strongly interacting phase. 
    ...
    {\bf (K)} Rescaling the ratio of $r_L/r_N$ using \eqref{eq:trans-rc-full-1}  collapses the transition point, % on similar curves with phase transition happening around near $1$, 
...
It is remarkable, however, that despite the different mechanisms the two models employ, %and the visually striking differences in the obtained layouts, 
...
that together occupy the volume $ V_N = 4 \sqrt{2} N r_N^3/3 $ (\ref{ap:phase-trans}). % where $\alpha = 4\sqrt{2}/\pi$. % appears because close-packing of spheres leaves some gaps between them. 
% Links also occupy space and 
...
% The volume occupied by links emerging from a node $i$ of degree $k_i$ consist of two constraints: the cylindrical volume of links, and the sphere of radius $r_0$ around  node $i$ where the $k_i$ links overlap. 
...
\end{linenumbers}
%\begin{linenumbers}
% Outside of the sphere of radius $r_0$, links occupy an approximately cylindrical volume. % in the weakly interacting phase.   
% The average length of the links depends on the  network topology. 
% For scale-free and random networks with no spatial symmetry $R_L = c N^{1/3} r_N$ with $c\approx 3/4$ (\ref{ap:phase-trans}), 
% For lattices and random geometric graphs only the unit cell should be analyzed (SI \ref{ap:phase-trans}).  
\outNim{
For a node of degree $k$ the volume occupied by the links  
\begin{align}
    ...
    \label{eq:VL0}
\end{align}
Setting $V_N = \sum_k V_L(k)$ to the link volumes will determine the phase transition, depending on which one occurs at smaller $r_L/r_N$ ratio. 
Thus, for a node of degree $k$ we have
\begin{align}
    % \alpha \pi N r_N^3 &\approx \pi r_L^3 \pr{c N^{1/3}L  {r_N \over r_L} 
    % - { 1\over 3}N \be{k^{3/2}}}%\cr
    % 0 &= 
    \alpha N \pr{{r_N\over r_L}}^3 -c N^{1/3} L {r_N \over r_L} 
    + { 1\over 3}N \be{k^{3/2}} = 0 
    \label{eq:trans-full0}
\end{align}
...
The degree becomes irrelevant only in the limit of $N^{-2/3} L \to \infty$, which is satisfied for any connected graph when $N\to \infty$, because $L> N-1$ and so $N^{-2/3} L > N^{1/3}$.  
Thus, the asymptotic solution becomes
\begin{align}
    \tilde{r}^c \equiv {r^c_L\over r_N} = \sqrt{{\alpha \over c}}%c^{-1/2} 2^{5/4} 
    N^{1/3}L^{-1/2} %\approx 2.75 \times N^{1/3}L^{-1/2}
    \label{eq:trans}
\end{align}
In Erd\"os-Renyi random graphs, the value of $c$ can be calculated form the uniformity of the connectivity (SI \ref{ap:phase-trans} eq.\eqref{eq:RL}) to be $c=3/4$. 
This yields $ \sqrt{{\alpha \over c}} \approx 1.55 $.
Eq. \eqref{eq:trans-full0} can also be solved exactly to find % a more accurate estimate of the transition point. 
% It has the form 
% \begin{align}
...
% leading to the 
% degree-dependent condition for the transition for a node of degree $k$ follows  
\begin{align}
    % \rho_+(k) 
    % &=\frac{A^{\frac{2}{3}} + 12 a}{6 \sqrt[3]{- A}}, &
    % A& = 12 \pr{9 b + \sqrt{(9b)^{2}- 12 a^{3} }}\cr
    \tilde{r}^c_L(k) = {r^c_L\over r_N} 
    &=\frac{6A^{{1\over 3}}(k)}{A^{\frac{2}{3}}(k) + 12 B(N,L)}, %\cr 
    % \mbox{where: }A& =- 12 \pr{{ 3k^{3/2}\over \alpha } + \sqrt{{ 9 k^{3}\over \alpha^2 }- 12 B^3 }},\quad B = {c\over \alpha}{L\over N^{2/3}}
    % \label{eq:trans}
...
%From \eqref{eq:trans-rc-full-1} we are able to find 
%The phase transition point for the whole network is obtained by averaging over the network's degree distribution. 
% the standard deviation of $\tilde{r}_c(k)$ over the degree distribution. 
...
% We also show that (SI \ref{ap:phase-trans}) in the thermodynamic limit where $N\to \infty$ \eqref{eq:trans-rc-full-1} reduces to our previous approximation $\tilde{r}_c = \sqrt{{\alpha \over c}} N^{1/3}L^{-1/2}$. 
...
% \end{equation}
% This predicts that w
While networks with different $N$ and $L$ transition at different $r_L/r_N$ ratios, if we scale $r_L/r_N$ with $\be{\tilde{r}^c_L}$ %from \eqref{eq:trans-rc-full-1} 
the phase transition occurs near one for all networks.
Using the order parameter $d\log\be{l}/d\log r_L $, %(the expansion exponent), 
the data collapse of Fig. \ref{fig:trans}K confirms the validity of \eqref{eq:trans-rc-full-1}, 
supporting the fact that the transition has its roots in excluded volume interactions. %: {  the logarithmic slope of total link length is negligible in the weakly interacting regime, but it rises to one in the strongly interacting phase.}
...
% This is reminiscent of vitrification in glass transitions \cite{gibbs1958nature}, i.e. the expectation 
% It is akin to a second-order phase transition, showing the characteristic growth of the relaxation time (Fig. \ref{fig:phase-compare}) and the discontinuity in the expansion exponent\footnote{ The expansion exponent is related to the second derivative of the elastic potential $V_{el}\sim \be{l^2}$.}. 
% A key point about vitrification is 
% This is indeed what we observe, 
Note that after rescaling of the transition point, networks of various sizes collapse onto the same curve with the same width % for the smooth transition curve 
...
% Vitrification is also consistent with the observation made below, that the strong phase exhibits gel-like properties.  

\outNim{
The transition exhibits properties of a ``Vitrification'' (glass transition \cite{gibbs1958nature}). It is akin to a second-order phase transition, showing the characteristic growth of the relaxation time (Fig. \ref{fig:phase-compare}) and the discontinuity in the expansion exponent\footnote{ The expansion exponent is related to the second derivative of the elastic potential $V_{el}\sim \be{l^2}$.}. 
A key point about vitrification is that the transition occurs over a finite range of $r_L/r_N$, regardless of the network size. 
This is indeed what we observe, as after the scaling by the transition point, networks of various sizes collapse onto the same curve with the same width for the smooth transition curve (Fig. \ref{fig:phase-compare}K, see \ref{ap:trans-width}), which is well described by a logistic curve in terms of $\log(r_L/r_N/\tilde{r}^c_L)$, predicting the width of the transition region near the transition point (Fig. \ref{fig:phase-compare}A--D).
Vitrification is also consistent with the observation made below, that the strong phase exhibits gel-like properties.  
\outNim{
{\color{red}
As we show in \ref{ap:phase-trans},  \eqref{eq:trans-rc-full-1} also allows us to estimate finite-size effects induced by the underlying degree heterogeneity, 
}%%%%red 
}%%%%
}%%%out
...
\begin{figure}
    ...
    % \includegraphics[width=.9\columnwidth]{fig-09-19/3D-stress-101217.pdf}
    %\includegraphics[width=.9\columnwidth]{fig-09-19/3D-stress-110317.pdf}
    %\includegraphics[width=.7\columnwidth, trim=0 1cm 0 5cm]{fig-09-19/3D-stress-pics-041218.pdf}
...
Therefore, %$D=3$ is the lowest dimension where any link crossing can be resolved and also the highest dimension where it is a problem at all.
...
% As we do live in 3D, the phase transition reported in the paper remains highly relevant to us. 
...
This implies that in these networks the average neuron length scales as $ \be{l} = V_w/A_w \propto r_L $ with the axon thickness, a behavior accurately predicted by the strongly interacting phase % and absent in the weakly interacting phase 
(Fig. \ref{fig:phase-compare}B). 
{ 
% Note that instead of individual neurons and axons, 
...
% There is evidence that most axons in the white matter follow a trajectory established by ``pioneer axons'' \cite{raper2010cellular}. 
% Therefore, much of the white matter consists of large bundles of axons, not individual ones. 
% There are between a dozen to a few hundred anatomical regions in the gray matter, depending on the scale we consider \cite{rubinov2015wiring}.
...
% The fact that the predicted scaling in the strongly interacting phase agrees with the empirical data offers further evidence of this link. 
% while scaling alone does not prove any relation, it is important to note that our findings indicate that the primate brain {\em is not} compatible with the strong phase, whereas the rodent brain is. 
% This suggests that to understand the primate brain we may need to consider other network characteristics, perhaps unique to the primate brain and neural networks. 
...
%}
%In other words,  
Thus, \eqref{eq:Vfull}--\eqref{eq:Langevin0} offers an appropriate modeling framework to capture the layout geometry of dense neuronal networks, generating an economic spatial layout %\cite{bullmore2012economy,sporns2004organization,kotter2001connectional}
...
% Note that the scaling does not apply to primate brains (where $V_w\propto A_w^{1.25}$ \cite{ventura2013different}), indicating the presence of other mechanisms not accounted for by our geometric model.
}
...
\begin{figure}
    ...
    \caption{\scriptsize Left: The behavior of the number of crossings as a function of link thickness. Trivially, once the thickness becomes comparable to the network size (when ``thickness/net. size'' becomes order 1) the crossings saturate and most links cross. The region in which trying to make links avoid each other is much smaller. It extends from zero thickness up to where thickness / net. size is of order $1/\sqrt{N_e}$ with $N_e$ being the number of links in the network. In the BA network with 100 nodes and $m=3$ considered here $1/\sqrt{N_3} \approx 0.06$, which is a safe lower limit for the region where avoiding crossings is possible. Right: a zoomed-in version of the dashed-red box on the lest highlighting this region of validity of avoiding crossings. The legends show a good linear fit to the data up to thickness / net. size = $0.2$.}
    \label{fig:full-range} 
\end{figure}
We show below that randomly oriented cylindrical links will result in a linear trend for the number of crossings versus link thickness. % that under such a distribution the number of link crossings grows linearly with link thickness\footnote{Assuming cylindrical links. % with circular cross-section.}.
...
The number of links should be of order $2N$ because each new node brings in exactly two new links. 
...
\begin{figure}
    ...
    \caption{\scriptsize Caption}
    %\label{fig:phase}
\end{figure}
}
Simulations of Barabasi-Albert and Erd\"os-Renyi networks layed out in 3D using the Fruchterman-Reingold forced-directed layout seems to suggest that in this setting the qualitative behavior of these networks for different values of parameters is very similar. 
...
\begin{figure}
    ...
    \caption{\scriptsize Simulations of number of crossings in BA and ER networks, when laid out using FDL.
...
In particular, we first show that if the links were arranged parallel to each other, but at random distances from each other, the number of crossings among them would grow quadratically with the thickness, inconsistent with the observed trend in FDL. 
...
\begin{figure}
    ...
    \caption{\scriptsize \textbf{Left:} The projection of the line through the center of an link \textsf{\textbf{ e1}} on the plane normal to link \textsf{\textbf{ e2}} which crosses the link at point \textsf{\textbf{ n2}}. Because links are randomly oriented, the distance of \textsf{\textbf{ e1}} and \textsf{\textbf{ e2}} is the distance of the blue line from the point \textsf{\textbf{ n2}}. The uniformity then means that the blue lines have uniform density at random angles on the plane and thus their number within distance $d$ is linear in $d$. \textbf{Right:} If links were parallel, the projection of \textsf{\textbf{ e1}} onto the plane would be a point. Thus the distance of the links is the distance of two points on the plane. Uniform spatial distribution in this case means we get uniformly distributed points on the plane and their number within a circle of radius $d$ grows as $d^2$.}
    \label{fig:crs1}
\end{figure}
...
The above-mentioned $d_p$ is the least distance between the two links only if $A$ crosses the plane $P$. Otherwise, one of the endpoints will be the closest point to the other line and we need a different strategy. Let's first figure out a way to see if $A$ crosses $P$ or not. To this aim we will look at the projections of $d_1= A_1-B_1$ and $d_2 = A_2 - B_2$ along the normal $\hat{n} $. Regardless of which endpoints of $A$ are being connected to which on $B$, if the projections of $d_1\cdot \hat{n}$ and $d_2\cdot \hat{n} $ have the same sign it means $A$ did not cross $P$ and if they have opposite signs or one is zero $A$ will have crossed $P$. Thus  
...
We will begin by deriving  mathematically how to find the shortest path between two points, in the presence of obstacles, showing that it is equivalent to stretching a rubber band between them.
...
We then combine all the potential energy terms and derive the equations of motion in a strongly dissipative medium, to make sure the system moves to a local minimum energy state. 
...
\begin{figure}
    ...
    \caption{\scriptsize methodology}
    \label{fig:forces}
\end{figure}
...
\begin{figure}
    ...
    \caption{\scriptsize \scriptsize{\bf Spherical vs Double-Ellipsoidal Potential:} If the forces between links $l,m$ depend on the Euclidean distance between link segments,
...
\begin{figure}
    ...
    \caption{\scriptsize Link curvature, which is equal to the inverse of the radius of curvature, is related to the angle between the tangent vector $\gamma'(s)$ and the change in the tangent vector $\gamma''(s)$. This can be estimated discretely in numerical simulations.}
    \label{fig:curvature}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{
    \scriptsize
    {\bf Simulations based on Mouse Brain White Matter Connectivity:} 
    The Mouse Brain Atlas data, collected by the Allen Institute, shows the trajectories of millions of axon bundles in the white matter of the mouse brain, connecting different regions of the gray matter. 
    We bundle these axon bundles even further to find the total connectivity of large anatomical brain regions, yielding a weighted network of connectivity of brain regions, as well as trajectories of bundles. 
    We then discard the trajectory information, keeping solely the weights (used as link thicknesses) and the location of brain regions (used as nodes). 
    We use this weighted network and run ELI on it, keeping nodes fixed, to generate simulated mouse brains with optimized wiring in 3D. 
    ...
    The top figure shows the top view of a simulation. 
    There are a total 20,000 weighted links, connecting about 700 brain regions at this level of bundling. 
    (A) Side view of the highest level of bundling resulting in 26 large brain regions and 700 weighted links, while (B) one lower level in hierarchy of brian regions, yielding 700 nodes and 20,000 links.  
    }
    \label{fig:brain}
\end{figure}
\outNim{
\begin{figure}[ht]
    ...
    \caption{
    \scriptsize
    {\bf Simulations based on Mouse Brain White Matter Connectivity:} Depending on the level of detail by which we define anatomical regions in the brain, we obtain different coarse-grained networks of brain connectivity. 
    (A) shows the highest level (maximum bundling) mao with large brain regions with 700 links, while (B) shows one lower level in hierarchy of brian regions, yielding 20,000 links.  
    }
    \label{fig:brain}
\end{figure}
}%%%%
...
For non-modular networks, the best heuristic algorithm would be to use k-d tree to partition the space to only resolve crossings within the partitions. %, because our forces are short-range. 
...
Intertwined links in 3D, however, can form of knots on the links, because the links remain effectively 1D objects, %: in 3D two one dimensional objects cannot always pass by each other, 
whereas 
%If we consider two thin links, 
...
So, $D=3$ is the lowest dimension where any link crossing can be resolved by moving links apart, and also the highest dimension where crossing can lead to knots and topological defects. %it is a problem at all.
% We live in 3D, and thus the phase transition reported in the paper remains highly relevant to us. 
}
...
During the dynamics one side of the network may fall into one minimum configuration and the other side into the reflected configuration, resulting in a twist at the boundary of the two layouts. %somewhere in the layout. 
...
% So, as expected, up to the numerical factor of $4/3$ our prediction for the phase transition seems correct.
...
% This expression for the link volume  $V_L$ will be valid as long as the second term is smaller than the first, meaning
...
The volume occupied by $N$ close-packed spheres is %not $4\pi N /3 r_N^3$, rather 
...
Thus, for the whole network we have % node of degree $k$ we have
\begin{align}
...
% In Erd\"os-Renyi random graphs, the value of $c$ can be calculated from the uniformity of the connectivity \eqref{eq:RL} to be $c=3/4$. 
}%%%%%%

\subsection{Transition in Lattice and Random Geometric Graphs}
%{\color{red}
The derivation of eq. \eqref{eq:trans-rc-full-1} assumes that links of node $i$ are not only extending to nodes spatially close to $i$, but also to distant nodes. % the distributed at all distances from the node. 
...
Thus in lattices and RGG, the transition point %and the spread of the transition region 
...
\outNim{
{\color{red}
Similarly in RGG, because of the translational symmetry all nodes are approximately similar and the network can be broken down to a number of repeating unit cells. 
First of all, both 2D and 3D RGG show the same phase behaviour as the rest
%s the transition point for networks
}%%%%%
\begin{figure}
    \centering
    ...
    Both 2D and 3D RGG behaves the same as the other networks we examined in 3D. 
    }
    \label{fig:RGG2}
\end{figure}
} 
...
If the hub node is placed at the center of the layout and if the nodes are occupying a volume with total radius $R_N\sim N^{1/3} r_N$, the average link length (SI 9) is $R_L = 3/4 R_N$. 
...
\begin{figure}[ht]
    ...
    \caption{\scriptsize {\bf Spatial heterogeneity of number of links in the neighborhood of nodes: }
...
Also, note that, as we explain in SI 7.A, a degree $k$ node in a an unweighted network has overlapping links up to radius $r_0\sim \sqrt{k} r_L/2 $ and to talk about the neighborhood of the node we have to talk about a region with radius larger than $r_0$.
...
A random graph clearly has a mostly uniform node distribution in space (except near boundaries of course), as all nodes look the same.
...
\section{Effect of Parameters on the Transition Point }

\outNim{
\subsection{Verifying the Scaling with $N$ and $L$}
To see if the predicted transition point behavior $\tilde{r}^c \propto N^{1/3}L^{-1/2}$ is accurate, we test it once on networks with a fixed number of nodes, $N=50$, and on networks with approximately fixed $750<L<1250$. 
