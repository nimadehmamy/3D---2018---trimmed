...
%\begin{table}%[]
    ...
    \hline
        ...
        \hline 
Abstract (words) & 200-300 &  500 & 314 \\
\hline 
Summary & 1 sentence ``we show'' & 150 words & - \\ 
\hline 
Text (words) & 1500 & 3000 & 2200\\
\hline 
Figures & 3-4 small & 5-6 normal & 4 large \\
\hline 
Pages & 4 & 5 & $\sim 3$ text, no figs \\
\hline 
References & $< 30$ & - & 37 \\
\hline 
    \end{tabular}
...
% \begin{document}
{\bf 
In many physical networks, from neurons in brain \cite{kasthuri2015saturated,oh2014mesoscale} to bundles of cables carrying power in infrastructural systems, underground hyphal networks \cite{friese1991spread,bago1998architecture}, the nodes and links are physical objects unable to cross each other.
These non-crossing conditions constrain their layout and physical embeddedness and also limit how these networks form, evolve and function. 
Currently, we lack models to describe such physical networks and existing network layout algorithms are variants of the Force-Directed Layout (FDL) algorithm \cite{kamada1989algorithm,davidson1996drawing,fruchterman1991graph,barnes1986hierarchical}, which assume dimensionless nodes and links, and %, ignoring their physical reality. % of the nodes and links. 
hence are unable to capture the characteristics of physical networks. 
%Virtually, all network layout models in use .
%The FDL cluster network modules \cite{noack2009modularity}. 
%, thereby pronouncing them. 
%Additionally, while in 2D FDL often leads to tangled-up layouts with modules intertwined, this problem largely disappears in 3D% thanks to the extra dimension
% \footnote{While in 2D only planar graphs can be laid out without crossings, in 3D any network can be laid out without any link crossings}.
%As FDL assumes dimensionless nodes and links, node overlap or link crossing have no impact on the layout. 
%Consequently, FDL-based models 
As such, FDL is inherently incapable of describing the properties of densely packed physical networks, like neurons in the brain.
Here, we develop a modeling framework inspired by polymer dynamics that accounts for the physical reality of nodes and links, allowing us to 
understand how the non-crossing conditions affect the network layout and characteristics. 
We find that for small link thicknesses networks can avoid crossings via local bendings, without the need to alter the overall network layout. 
Once, however, link thickness reaches a critical threshold a strongly interacting phase emerges, in which  excluded volume interactions determine the network layout. 
We analytically derive the transition point, and show that multiple geometric quantities, from total link length to link curvature, change their scaling behavior in the strongly interacting phase.  
We also document a deep universality, finding that the scaling law characteristics of the obtained layouts are largely independent of the underlying network topology. 
The observed layout transition  affects the physical properties of the network, with networks displaying a solid-like response to stress in the weakly interacting phase, whereas they behave in a gel-like fashion in the strongly interacting phase. 
These findings not only unveil the fundamental constraints obeyed by physical networks, but have direct implications for the layout of mammalian brains and for 3D printing of realistic networks.
} 
\end{abstract}
\maketitle
%\end{document}
...
\begin{figure}
    ...
    This decreases the energy penalty of jumping from one local minimum to another and makes it more likely for the system to relax completely.
    %\scriptsize 
    {\bf (D)} When the links are thin, i.e. $r_L\ll r_N$, they interact only weakly with each other --the weak exclusion regime. 
    ...
    {\bf (E)} When links become thick they exclude each other strongly --the strong exclusion regime. 
...
To accurately lay out physical networks, we must arrange the links and the nodes in such a way to avoid the links and nodes crossing each other, while minimizing the total link length. 
We must therefore find the shortest path for each link, even when the straight path is obstructed by other nodes and links, a problem similar to stretching a flexible rubber band between flexible obstacles (see \cite{novikov1984}, and SI sec.\ref{ap:affine} for proof). 
To achieve this we have built a model in which each force in the network is determined by the gradient of the total potential energy,  
\begin{align}
 ...
 \label{eq:Vfull}
\end{align}
where $V_{el}$ represents the total elastic potential of all links $l=1,...,L$. 
...
We use FDL to set the initial node positions and we systematically explore two versions of the model with different constraints: 
(i) In the Elastic Link Model (ELI), which corresponds to $\lambda_N\to \infty$, the node positions are fixed in the original FDL-dictated positions and only the links are allowed to reorganize themselves; 
...
Equations \eqref{eq:Vfull}--\eqref{eq:Langevin0} represent a special case of general manifold dynamics, used to model polymers \cite{mezard1991replica}. 
The network has an uneven potential energy landscape \cite{parisi2002physical} with a multiple local minima, hence identifying the globally optimal configuration is NP hard (SI sec.\ref{ap:np}).
...
Fig. \ref{fig:crs-lat}E shows how the noise in FUELF often helps the system tunnel through the finite potential walls to escape local minima and find the correct 3D configuration of a lattice (SI sec.\ref{ap:np}). 
To understand the challenges ELI and FUEL must address, let us start with 
%If we do consider 
FDL, which assigns spring-like attractive forces between linked node pairs, and repulsive forces between all node pairs to ensure that the network does not collapse. 
As FDL ignores the physical dimensions of the nodes and links, with increasing the link thickness $r_L$, it results in multiple link and node crossings (Fig. \ref{fig:crs-lat}A). The number of such conflicts increases linearly with link thickness $r_L$, and saturates for large $r_L$ (Fig. \ref{fig:phase-compare}C), a behavior accurately predicted by a simple geometric model (SI, sec. \ref{ap:cross}) \cite{kabourov2015spring}. 
To avoid these conflicts, we applied ELI and FUEL to a wide range of network topologies, sizes, and densities. We find that the obtained network layouts undergo a remarkable geometrical transition as we increase the link weights
\outNim{ % 071917
\begin{figure}
    \centering
...
(Fig. \ref{fig:phase-compare}A, B). 
Next we discuss the layout characteristics, before and after this transition.% show layouts generated using ELI (A) and FUEL (B), also documenting a qualitative structural change as we increase $r_L$, as we discuss next.
\begin{figure}
\centering
%\vspace{-2cm}
...
\caption{%3D layouts  using ELM and E-ELM, and their phase diagram.
         ...
         ({\bf E}) While total Link curvature, normalized by the average link length $\be{l}$, of BA and ER overlap almost completely in both ELI and FUEL (bottom), the lattice does differ from them in the ELI. }    
    \label{fig:phase-compare}
\end{figure}

{\em Weak Exclusion Phase}: For small link thicknesses the ELI and FUEL layouts are largely indistinguishable from the initial FDL layout.
Indeed, we find that at low $r_L$, the average link length $\be{l}$ remains unchanged even as we increase $r_l$ by orders of magnitude.
This is unexpected, given that the number of potential conflicts in the FDL algorithms goes up ten fold (Fig. \ref{fig:phase-compare}C). 
The unchanged $\be{l}$ indicates that the links can avoid the increasing number of conflicts by local bending, without increasing $\be{l}$.
We also measured the average curvature of each link, finding that it remains negligible in the weak exclusion phase, indicating that the links remain largely straight. 

{\em Strong Exclusion Phase}: We do observe a dramatic, visually detectable change in the layout once $r_L$ exceeds a critical value (Fig. \ref{fig:phase-compare}A,B). This change is accompanied by changes in behavior of $\be{l}$, which increase linearly with $r_L$. 
In this phase local bending cannot resolve the conflicts, forcing the links to drastically increase their length. 
In ELI, where the node positions are fixed, the links can only reach their end-nodes by taking long convoluted routes outside the network.
In FUEL, with flexible node positions, the links push the nodes away from each other. 
It is remarkable, however, that despite the different mechanisms the two models employ, and the visually striking differences in the obtained layouts, the scaling of $\be{l}$ in ELI and FUEL is largely indistinguishable. 
We also observe a rapid increase in the curvature  at $r^c_L$, reflecting the emergence of systematically curved links for large $r_L/r_N$. 
However, once the radius of curvature is scaled by  $\be{l}$ (SI \ref{ap:curvature}), we find that it becomes constant for all networks in the strong exclusion phase (Fig. \ref{fig:phase-compare}E). 
...
% Together with linear growth of  $\be{l}$  (Fig. \ref{fig:phase-compare}D), this implies that layouts in the strong exclusion regime at different $r_L/r_N$ are similar, once scaled by layout size.

The origin of the observed transition is revealed by estimating the critical $r_L^c$.
...
Hence, the radius $R$ of the ``bounding sphere'' engulfing all nodes is well approximated by the volume of $N$ loosely placed spheres of radius $r_N$, i.e. $ R^3 \sim Nr_N^3 $ (Fig. \ref{fig:trans}A). 
When the total volume of the links exceeds the volume of the bounding sphere, the links cannot fit within the space between nodes, forcing a change in the layout.  
The new layout will be dominated by the total link volume, implying that the cross sectional area of the layout $\pi R^2$ must be comparable to the cross section of all links $ L r_L^2 \sim R^2$ (Fig. \ref{fig:trans}B). 
\outNim{% 092517 
\begin{figure}
    \centering
    % \includegraphics[width=\columnwidth]{fig-09-19/transition.pdf}
    ...
    % \includegraphics[width=\columnwidth]{fig-09-19/transition-3.pdf}
    % \includegraphics[width=\columnwidth]{fig-09-19/3D-transition.pdf}
    \includegraphics[width=\columnwidth]{fig-09-19/3D-trans-092517.pdf}
...
The link thickness $r^c_L$ marking the transition between the two layouts corresponds to   $r_L$ when the two estimates for $R$ are comparable, resulting in the critical relative radius 
\begin{equation}
    \tilde{r}_c \equiv {r^c_L\over r_N} \sim N^{1/3} L^{-1/2}. \label{eq:trans}
\end{equation}
We find that this prediction offers a reasonable estimate of the transition point for both ELI and FUEL. Given its fundamental geometrical nature, we expect to apply to all models that consider the physical nature of nodes and links (Fig. \ref{fig:trans}C). 
Indeed, while networks of different $N$ and $L$ and different degree distributions transition at different points, if we scale the $r_L/r_N$ ratio by \eqref{eq:trans} the phase transition occurs near 1 for all networks (Fig. \ref{fig:trans}D), supporting the validity of \eqref{eq:trans}.   
...
suggests that the phase diagram of Fig. \ref{fig:phase-compare} is universal, being independent of the network topology and the degree distribution. Most importantly, the accuracy of \eqref{eq:trans} confirms that the transition documented in Fig. \ref{fig:phase-compare}A,B,D has its roots in an excluded volume interactions while the link volume is negligible in the weakly interacting regime, they dominate the layout in the strongly interacting phase. 

The documented layout transition also alters the obtained network's physical properties, when subject to external forces.
A network's response to external forces is determined by the Cauchy stress tensor $T_{\mu\nu} \equiv \ro_\mu\ro_\nu V$ \cite{irgens2008continuum} (SI sec.\ref{ap:stress}), which in turn depends on the physical and material properties of the nodes and the links. 

In the weak exclusion regime the links are mostly straight, hence the node terms $V_{NN}$ and $V_{NL}$ dominate the total stress. 
As the nodes are surrounded by a varying number of other nodes, the stress does not spread uniformly in all directions, but has shear (off-diagonal) stress components, a common feature of solids.
%In other words, networks in the weak exclusion phase have solid-like properties. 
In the strong exclusion phase, the links fill up the space,
and link contributions $V_{el}+V_{LL}$ in \eqref{eq:Vfull} dominate $T_{\mu\nu}$, 
%The adjacent link segments will be at a distance $|x_l-x_m| = \be{r} = 2 r_L$, hence space-filling and uniformity 
resulting in a diagonal total stress tensor (SI sec.\ref{ap:stress}) 
\begin{equation}
    \mbox{}\int d^3x  T_{\mu\nu} \approx \left[ k + {L \be{l}^2\over 4r_L^2}   \right]\delta_{\mu\nu} \label{eq:Tstrong}
,\end{equation}
%
describing a hydrodynamic stress with pressure $ p =  k + {L \be{l}^2/( 4r_L^2)}$. 
In other words, networks in the strong exclusion limit are expected to display a fluid-like response to external stress. 
%Given that the link segments do not diffuse freely, a
A more accurate description is that of a gel, %like a pool of polymers whose components 
...
We measured the total stress for varying $r_L/r_N$, observing a transition from a roughly constant stress in the weak exclusion regime to a monotonically increasing total stress in the strong exclusion regime (Fig. \ref{fig:stress}C). 
The transition occurs around $\tilde{r}_c$ predicted by \eqref{eq:trans}, indicating that the change in the system's response to stress is indeed caused by the layout transition from a node to a link-dominated geometry.  
Equally important, as we rotate the network, we
find that the total stress $ \sigma_\parallel/ \sigma_\perp$ ratio displays large fluctuations in the weak exclusion regime, a behavior expected in anisotropic solids. 
The fluctuations largely vanish at the transition $\tilde{r}_c$, reaching the hydrodynamic ratio $\sigma_\parallel/\sigma_\perp = 1/\sqrt{2}$  (Fig. \ref{fig:stress}D), as expected for gels or fluids under pressure. 
...
%\newpage
%\section*{Discussion}
In summary, the developed modeling framework predicts the existence of two distinct phases: a weak exclusion phase, where the overlap between the nodes and links can be avoided via local link rearrangements; and a strong exclusion phase, whose layout is dominated by %link crossings.
%{\color{red} ALL topologies in one place} In conclusion, w
%The transition between the two phases is driven by 
excluded volume interactions. 
We find that in the  two phases the network layout has distinct scaling and physical characteristics: Networks in the weak exclusion phase have solid-like properties, while
those in the strong exclusion phase behave like a gel. 

Both phases have potentially important applications. 
Indeed, weak exclusion phase could open novel avenues for three dimensional computer rendering of large networks and 3D printing, offering insights about the inner structure of a complex system. 
...
As FDL is blind to the physical nature of the nodes and links, it leads to excessive node-link overlaps obstructing the true wiring diagram and the underlying geometry of the network. 
Applying FUEL to the same network, if we choose $r_L$ so that the layout remains in the weak exclusion phase, we obtain in layouts that reveal the underlying link structure and clearly identify the inherent community characterizing the network. 
%As visibility is key for visualization purposes, the weak exclusion phase results in more appealing layouts and is more suited than the strong exclusion phase for visualization (Details on good choice of parameters in SI \ref{ap:params}). 
To show the potential relevance of the layouts we 3D printed the obtained network, offering an unprecedented opportunity to inspect its inner structure. 
...
Note that such layouts, to be instructive, need to stay in the weak exclusion phase, as in the strong exclusion phase, the inner structure is obstructed by the total link volume.
...
The strong exclusion phase has direct relevance to the the brain, a three dimensional physical network where the close-packing of the axons is critical to their ability to form synapses \cite{stepanyants2002geometry,rivera2011wiring}. 
Given the exceptional number of connections each neuron forms, the wiring of the brain is expected to be economical  \cite{bullmore2012economy,sporns2004organization,kotter2001connectional} while overlapping link crossings features well captured by our modeling framework. 
The relevance of the strong exclusion phase for the brain network geometries 
%Network layout models that are economical and efficient are, therefore, a necessary steppingstone towards understanding the wiring of the brain. %, and the limitations its physical link structure imposes on its wiring diagram. %Take, for example, 
%The size of the brains of rodents and many other non-primate mammalian species 
%that differ from each other in the number of neurons and sizes of different brain regions
%with similar brain structures 
\cite{azevedo2009equal, herculano2012remarkable, herculano2014brain}
%has been observed to follow 
is supported by isometric scaling laws, i.e. the obstructed volume and that the area of the white matter, $V_w$ and $A_w$, are related as $V_w\propto A_w^{1.5}$ \cite{herculano2012remarkable}. 
This implies that in these neural networks the average neuron length scales as $ \be{l} = V_w/A_w \propto r $, a behavior predicted to be present only in the strong exclusion phase but absent in the weak exclusion phase (Fig. \ref{fig:phase-compare}D). 
We therefore believe that the modeling of the layout of the dense neuronal network will have to resort to frameworks like \eqref{eq:Vfull}--\eqref{eq:Langevin0}, offering economic spatial layouts while respecting the non-crossing conditions neurons must obey.  
...
\begin{figure}
    ...
    \label{fig:crs}
\end{figure}
It is easy to prove that under such a distribution the number of link crossings grows linearly with link thickness\footnote{Assuming cylindrical links. % with circular cross-section.
