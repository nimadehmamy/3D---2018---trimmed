...
Virtually, all network layout models in use are variants of the Force-Directed Layout (FDL) algorithm \cite{kamada1989algorithm,davidson1996drawing,fruchterman1991graph,barnes1986hierarchical}, which assigns spring-like attractive forces between linked node pairs, hence minimizing the distance between them, and adds repulsive forces between all node pairs to ensure that the network does not collapse to a point  
 \cite{kabourov2015spring}.
Given that FDL assumes dimensionless nodes and links, node overlapping or link crossing have no impact on the layout. 
Consequently, these models are inherently incapable of capturing the properties of densely packed physical networks, like neurons in the brain \cite{kasthuri2015saturated,oh2014mesoscale}, underground hyphal networks \cite{friese1991spread,bago1998architecture}, or a bundle of wires in computer systems.
If, however, we do consider the physical dimensions of the nodes and links, link crossings are unavoidable (Fig. \ref{fig:crossings}A). 
We find that the number of such crossings increases linearly with link thickness $r_L$, and saturates for large $r_L$ (Fig. \ref{fig:crossings}B), a behavior analytically predicted by a simple geometric model (SI, sec. \ref{ap:cross}) . 
In other words, in physical networks a proper layout must systematically avoid increasing number of crossings.
...
$V_{LL}$ will then describe the interaction of pairs of ellipsoids, which can conveniently be achieved by replacing the simple Euclidean norm $|\Delta x_{lm}| = |x_l-x_m|%\sqrt{(x_l-x_m)^T(x_l-x_m)} 
...
%Also, the discreteness of the numerical simulations alters the potential energy barriers between local minima and allows for easier transition between them. 
...
% Note that, in a connected network, $r^c_L$ is always much smaller than the layout diameter, which is approximately where the number of link crossings saturates (Fig. \ref{fig:phase-compare}C).
% For the networks in Fig. \ref{fig:phase-compare}, the transition point predicted by \eqref{eq:trans} is 
% $\tilde{r}_c \approx 0.2$.
...
%We observe a slightly higher curvature for the lattice, compared to a scale-free and a random network when using ELM in the strong exclusion limit. 
%This can be explained as follows.  
%In ELM, for lattices, with a local geometric structure, FDL results in a fairly good lattice layout, where all links are short. 
% In locally heterogeneous networks, which include the random and scale-free topologies, a mixture of long and short links are required in FDL. 
% As a result, in the strong exclusion regime in ELM, where the lack of space pushes the links out radially, lattice links have to bend back sharply to reach neighboring nodes, while in random and scale-free networks long links can go around the whole layout from the outside, without having to bend sharply.
...
%Moreover, if the network behaved as a fluid, stress would spread uniformly in all directions. 
%This is not the case in the and 
%Thus, we conclude
...
All adjacent link segments will be at a distance $|x_l-x_m| = \be{r} = 2 r_L$, hence the space-filling and uniformity will result in a diagonal total stress tensor (SI sec. \ref{ap:stress}) %(integrated over the whole network) 
\begin{equation}
...
If, on the other hand, the ratio $ \sigma_\parallel/\sigma_\perp$ is orientation-dependent, varying as we randomly rotate the layout, the network %has different elastic moduli in different directions, and so it 
...
All these observations point to a relation between rodent brains and the strong exclusion phase, but a definitive conclusion requires detailed analysis of the white matter. %\footnote{
...
$\delta_{\mu\nu}\sum_{<ab>}\int ds_l ds_m \approx N_L \be{l}^2 $
...
We examined numerically whether the $ \sigma_\parallel/ \sigma_\perp$ ratio is constant in the E-ELM layouts.
We find that, as predicted by the theory, $\sigma_\parallel/\sigma_\perp$ fluctuates significantly for node stress at almost all (except at very high) link thicknesses, indicating that it does not satisfy the hydrostatic condition (Fig. \ref{fig:stress}D).%(SI sec. \ref{ap:stress}).
In contrast, the stress in links shows a remarkably constant  $\sigma_\parallel/\sigma_\perp$ ratio starting at thicknesses well below the phase transition, indicated by a red strip (SI). 
At very low thicknesses link stress too shows large fluctuations. 
