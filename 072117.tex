...
\hline 
Text (words) & 1500 & 3000 & 2400\\
\hline 
Figures & 3-4 small & 5-6 normal & 5 large \\
\hline 
...
% \begin{document}
{\bf 
In many physical networks, from neurons in brain to bundles of cables carrying power in infrastructural systems, the nodes and links are physical objects unable to cross each other.
These non-crossing conditions determine their layout and physical embeddedness  and limit how these networks form and evolve. 
Current network layout algorithms ignore the physical reality of the links and nodes, hence are inherently unable to model the characteristics of such physical networks. 
Here, we develop a model inspired by manifold dynamics that accurately captures the physical reality of nodes and links, allowing us to 
understand how they affect the layout of physical networks. 
We show that for small link thicknesses conflicts are rare, hence networks are in a weakly interacting phase, where can avoid crossings with local bendings, without altering the overall network layout. 
Once, however, link thickness reaches a critical threshold, we observed a transition to a strongly interacting phase, in which  excluded volume interactions fundamentally reshape the layout. 
We derive this transition point, and show that a set of geometric quantities, from total link length to link curvature, alter their scaling behavior at the predicted transition point. 
At the same time we document a deep universality in each phase, finding that geometric characteristics and scaling of the obtained layout are independent of the underlying network topology. 
Finally, we show that the transition point also affects the physical properties of the network: networks respond to external stress like a solid in the weakly interacting phase, whereas they behave in a gel-like fashion in the strongly interacting phase. 
These findings not only unveil the fundamental constraints physical networks must obey, but have direct implications in the layout of mammalian brains and for the 3D printing of realistic networks.
} 
\end{abstract}
\maketitle
%\end{document}
...
If we do consider the physical dimensions of the nodes and links, FDL leads to multiple link crossings or conflicts (Fig. \ref{fig:crs-lat}A), whose number increases linearly with link thickness $r_L$, and saturates for large $r_L$ (Fig. \ref{fig:crs-lat}B), a behavior accurately predicted by a simple geometric model (SI, sec. \ref{ap:cross}). 
\begin{figure}
    %\vspace{4cm}
    \centering
...
To accurately lay out physical networks, we aim to arrange the links and the nodes in such a way to systematically avoid these conflicts while minimizing the total link length for a fixed $r_L$. 
This requires us to find the shortest path each link must follow, when the optimal straight path is obstructed by other nodes and links, a problem similar to stretching a flexible rubber band between a series of obstacles (see \cite{novikov1984}, and SI sec.\ref{ap:affine} for proof). To achieve this we propose a model in which each force in the network is determined by the gradient of the total potential energy,  
\begin{align}
 ...
 \label{eq:Vfull}
\end{align}
where $V_{el}$ represents the total elastic potential of all links $l=1,...,L$, where each link is an elastic cylinder with radius $r_L$, experiencing both internal elastic forces and short-range external repulsive force from other links and nodes; $V_{NL}$ captures the node-link interactions at link endpoints. Nodes and links are prohibited from crossing each other's boundaries, a constraint ensured by a short-range repulsive force in
...
The lowest energy solution of \eqref{eq:Vfull} can lead to sharp bending of some links, which is avoided via a Gay-Berne \cite{gay1981modification,berne1972gaussian} potential employed in polymer physics \citep{everaers2003interaction,babadi2006coarse,mergell2003modeling,cleaver1996extension} (SI sec.\ref{ap:ell}).


To allow the system to relax to a low energy state without oscillations, we embed the network in a high viscosity medium. 
Therefore, the position of the nodes and links follow first order gradient descent equations of motion 
\begin{align}
...
We initialize the model by using FDL to fix the initial node positions. 
We explore two versions of the model. In the $\lambda_N\to \infty$ limit the node positions are fixed, hence only the links can reorganize themselves, obtaining the Elastic Link Model (ELI).
For finite $\lambda_N \sim \lambda_L$, both nodes and links are free to move, yielding the Fully Elastic Model (FUEL). 
...
The noise helps the system tunnelling through the finite potential walls to get out of local minima and reaching the familiar 3D lattice layout (SI sec.\ref{ap:np}).
\outNim{ % 071917
\begin{figure}
    \centering
...
Figures \ref{fig:phase-compare}A and 
B show layouts generated using ELI (A) and FUEL (B), documenting a qualitative structural change as we increase $r_L$. 

{\em Weak Exclusion Phase}: At small $r_L$ the ELI and FUEL layouts are largely indistinguishable from the initial FDL layout.
\begin{figure}
\centering
%\vspace{-2cm}
...
\end{figure}
We measured the average length of links $\be{l}$,
observing that at low $r_L$, the average link length $\be{l}$ remains unchanged despite orders of magnitude in $r_L$.
This is particularly surprising, given that the number of crossings goes up ten fold (Fig. \ref{fig:phase-compare}C). 
The unchanged $\be{l}$ indicates that in this {\em weak exclusion} regime the layout avoids the increasing number of conflicts by local bending, without significantly altering the total link lengths. To further characterize link structure,
...
{\em Strong Exclusion Phase}: Once $r_L$ exceeds a critical value, $\be{l}$ changes its behavior and increases linearly with $r_L$. 
...
What is remarkable, however, is that despite the different mechanisms the two models employ, and the visually striking differences in the obtained layouts, the scaling of $\be{l}$ of ELI and FUEL is largely indistinguishable. Beyond $r^c_L$ the curvature rapidly grows, reflecting the need for multiple and often significant bendings to avoid crossings. 
...
To understand the origin of the observed transition next we estimate the critical $r_L$.
...
In this case the layout is dominated by the total link volume, hence the cross sectional area of the layout $\pi R^2$ becomes comparable to the cross section of all links $ L r_L^2 \sim R^2$ (Fig. \ref{fig:trans}B). 
\begin{figure}
    \centering
...
We find that this prediction offers a reasonable estimate of the numerically obtained transition point for both models (Fig. \ref{fig:trans}C). Indeed, while networks of different $N$ and $L$ transition at different points, scaling the $r_L/r_N$ ratio by \eqref{eq:trans} places the phase transition near 1 on the x axis (Fig. \ref{fig:trans}D).     
...
suggesting that the phase diagram of Fig. \ref{fig:phase-compare} are independent of the network topology and the degree distribution. Most importantly the accuracy of \eqref{eq:trans} tells us that transition in the scaling and layout documented in Fig. \ref{fig:phase-compare}A,B,D has its roots in an excluded volume interaction driven by the links: in the strongly interacting phase such excluded volume interactions dominate the layout; while those interactions are negligible in the weakly interacting regime.
...
This is captured by the Cauchy stress tensor $T_{\mu\nu} \equiv \ro_\mu\ro_\nu V$ \cite{irgens2008continuum} (SI sec.\ref{ap:stress}).
Normally, $T_{\mu\nu}$ is determined jointly by the physical properties of the nodes and the links, and the material structure, in our case representing the network topology and layout. 
...
In a network environment nodes are surrounded by a varying number of other nodes, hence the stress does not spread uniformly in all directions, but leads to shear (off-diagonal) stress components. In other words, networks in the weak exclusion phase have solid-like properties. 
In the strong exclusion phase, where the links fill up the space,
the link contributions $V_{el}+V_{LL}$ in \eqref{eq:Vfull} dominates $T_{\mu\nu}$. 
The adjacent link segments will be at a distance $|x_l-x_m| = \be{r} = 2 r_L$, hence space-filling and uniformity resulting in a diagonal total stress tensor (SI sec.\ref{ap:stress}) 
\begin{equation}
...
Given that the link segments are not free to diffuse or move freely, a more accurate description is that of a gel, like a pool of polymers whose components are stationary, unless external forces act on them. 


To test the validity of the predicted solid-gel transition,
we placed the networks generated by FUEL between two walls compressing them in the $y$ direction by $10\%$ of its equilibrium height and measured the tensile forces, 
...
If, on the other hand, the ratio $ \sigma_\parallel/\sigma_\perp$ varies as we rotate the layout, the network possesses solid-like properties. 
\begin{figure}
    \centering
...
The transition occurs around $\tilde{r}_c$ predicted by \eqref{eq:trans}, indicating that the change in the system's response to stress is indeed caused by the excluded volume induced transition from a node to a link-dominated layout.  
...
These fluctuations largely vanish at the transition, where the ratio  reaches the hydrodynamic ratio $\sigma_\parallel/\sigma_\perp = 1/\sqrt{2}$  (Fig. \ref{fig:stress}D), as expected if we place a gel or a fluid under pressure. 

Three dimensional computer rendering of large networks can offer insights about the inner structure of a complex system. 
Given, however, the cognitive limits of interpreting and experiencing computer images in 3D, it has only limited ability to illustrate its the underlying network's true complexity. 
To experience the true dependencies of these networks, a true 3D physical models can offer unparalleled insights. 
The advantage of the proposed modeling framework is that it can be directly linked to the 3D printing technologies, allowing us to lay out in an optimal manner large networks, and  print them using 3D printers. 
We illustrate the outcome of such a process in Figure \ref{fig:3d-print}, where we show a 3D printed version of the Flavor Network \cite{ahn2011flavor}.
...
Modifications such as addition of weak long-range node repulsion and bundling of links can further enhance the visual aesthetics of FUEL in the weak exclusion.  
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{fig-09-19/3d-flavor-panel.pdf}%{fig-09-19/flavor-3d-print.jpg}
    \caption{\scriptsize 
...
\newpage
%\section*{Discussion}
In summary, the developed modeling framework to lay physical networks out in 3D predicts the existence of two distinct phases: a weak exclusion phase, where crossings can be avoided via local link bendings; and a strong exclusion phase, whose layout is dominated by link crossings.
%{\color{red} ALL topologies in one place} In conclusion, w
We find that the transition to be driven by excluded volume interactions and is universal, meaning that networks with different topologies and constraints display the transition at the same critical point, and behave similarly in the two regimes. 
We also find that while the strong exclusion phase behaves like a gel under stress, networks in the weak exclusion phase have solid-like properties. 

The consequences of the physical constraints in 3D embedding may potentially affect the dynamical and the functional properties of networks. Case in point is the brain, a three dimensional physical network where the proximity of the axons is critical to their ability to form synapses \cite{stepanyants2002geometry,rivera2011wiring}. 
...
Due to the enormity of the number of connections among neurons, the wiring of the brain needs to be economical \cite{bullmore2012economy,sporns2004organization,kotter2001connectional}. Network layout models that are economical and efficient are, therefore, a necessary steppingstone towards understanding the wiring of the brain, and the limitations its physical link structure imposes on its wiring diagram. 



Take, for example, the brains of non-primate mammalian species that differ from each other in the number of neurons and sizes of different brain regions \cite{azevedo2009equal, herculano2012remarkable, herculano2014brain}. 
It has been observed that brains of rodents \cite{herculano2012remarkable} and many other mammals \cite{herculano2014brain} follow isometric scaling laws, i.e. the volume and area of the white matter, $V_w$ and $A_w$, are related as $V_w\propto A_w^{1.5}$ \cite{herculano2012remarkable}. 
In other words, in rodents we expect the average neuron length to scale as $ \be{l} = V_w/A_w \propto r $, a behavior consistent with the strong exclusion phase (Fig. \ref{fig:phase-compare}D). 
