...
One is the other mathematical invariant, beside $\be{l}$  that one can get from links, %a set of paths in Euclidean space 
...
\begin{align}
    ...
    =& k \delta_{\mu\nu}\int dl \delta(x-x_a(l)) \cr
    &+ 
    \sum_{i\ne j} \int \int dl_a dl_b \delta(x-x_a(l_a))\frac{-4r^2\delta_{\mu\nu}+x_{i\mu}x_{i\nu}}{16r^4}
    A \exp\left[- {|\vec{x}_a-\vec{x}_b|^2 \over 4 r_0^2}\right]\cr
    &+ \sum_{I\ne J}\delta(x-X_i)\frac{-4r_N^2\delta_{\mu\nu}+X_{I\mu}X_{I\nu}}{16r_N^4}
    A \exp\left[- {|\vec{X}_i-\vec{X}_j|^2 \over 4 r_0^2}\right]
\end{align}
where the first row corresponds to the elastic stress in the links, the second to the link repulsion and the third to node forces.   
} %%%% 
